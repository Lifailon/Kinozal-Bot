...
`KINOPOISK_API="True"` - получение информации об актере будет из базы данных Кинопоиск (если **False**, до ссылка в Кинозал) \
...
`/search Рокки 2` - Поиск все фильмов (до 50 кнопок с ссылкой на фильм) \
`/search 1979 Рокки` - Поиск фильма с фильтрацией по году выхода \
`/actor Сильвестр Сталлоне` - Поиск по актеру
