...
По мимо этого используется файловая база в формате `JSON` (во избежании лишних зависимостей, файл `posting.json`) для анализа публикация на канале.
...
Я использую **HandyCache (Proxy)** в системе *Windows*, рядом с которым запущена бесплатная версия *VPN Hotspot Shield* в режиме **Split Tunneling** до сайта Кинозал (в таком режиме не будет доступен сервис *TMDB API*), при котором трафик до указанного сайта проходит через VPN туннель и не затрагивает другие сервисы, тем самым не ограничивая загрузку торрентов на хостовой машине. Вы также можете настроить выделенную виртуальную машину для подобного стэка (а также добавить в него *Wi-Fi* модуль для организации второй домашней сети) без необходимости использовать раздельное туннелирование.
