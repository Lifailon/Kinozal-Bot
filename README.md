...
С помощью бота вы получите удобный, и главное единый интерфейс для взаимодействия с торрент трекером [Кинозал](https://kinozal.tv) и базой данных [TMDB](https://www.themoviedb.org) для отслеживания даты выхода серий, сезонов и поиска актеров для каждой серии, а также возможность управлять торрент клиентом [qBittorrent](https://github.com/qbittorrent/qBittorrent) или [Transmission](https://github.com/transmission/transmission) на вашем компьютере, находясь удаленно от дома. В отличии от других приложений, предназначенных для удаленного управления торрент клиентами, вам **не нужно** находиться в той же локальной сети или использовать VPN.
...
Запуск в контейнере Docker является альтернативой настройки службы **systemd** и такой способ является **рекомендуемым**.
...
Рабочая директория (**WORKDIR**) соответствует параметру `path` в конфигурации и используются для синхронизации между локальным (для отладки) запуском и в контейнере, по этому **измените путь на свой** 💡.
